...
return'0'+s_num;else
return num.toString();}
$(document).ready(function(){var vm=new DayJournal(ko_data);ko.applyBindings(vm);$('.change-on-ready').trigger('change');if(window.location.hash!=""){$('a[href="'+window.location.hash+'"]').click();}
...
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
function InventoryRow(row){var self=this;self.item_id=ko.observable();self.purchase=ko.observable(0);self.sales=ko.observable(0);for(var k in row){if(row[k]!=null)self[k]=ko.observable(row[k]);}
self.opening=ko.computed(function(){if(self.item_id()){var item_temp=vm.inventory_item_by_id(self.item_id()).get_current_stock;return item_temp;}
else return 0;},vm);self.closing=ko.computed(function(){console.log(parseFloat(self.opening()),parseFloat(self.purchase()),parseFloat(self.sales()));return rnum(parseFloat(self.opening())+parseFloat(self.purchase())-parseFloat(self.sales()));});self.actual=ko.observable();self.difference=function(){return rnum(self.actual()-self.closing());};}
