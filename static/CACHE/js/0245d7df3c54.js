...
arrow_handling();$(document).ready(function(){$('.selection').select2();vm=new AttendanceVM();ko.applyBindings(vm);var monthNames=["january","february","march","april","may","june","july","august","september","october","november","december"];var objDate=new Date();$('.month-tab').click(function(){var id=$(this).attr('id').split('-')[0];window.month=id;ajax_request(id,vm.employee);});$("#input1").click(function(){$("#sele").click(function(){$("select.sel").select2();var mnth=monthNames[objDate.getMonth()]
var el="#"+mnth+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});$("#sele").click(function(){$("#input1").click(function(){$("select.sel").select2();var mnth=monthNames[objDate.getMonth()]
var el="#"+mnth+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});});function employee_ajax(obj){$.ajax({url:'/payroll/employees.json',dataType:'json',async:false,success:function(data){obj.employees=data;}});}
function AttendanceVM(){var self=this;employee_ajax(self);self.month=ko.observable('january');self.days=ko.observableArray();self.employee=ko.observable();self.attendance_type=ko.observable();self.hour_attendance=ko.observableArray();self.day_attendance=ko.observableArray();self.total_time=ko.computed(function(){var sum=0;for(var i=0;i<self.hour_attendance().length;i++){if(self.hour_attendance()[i].working_time()){sum+=get_minutes(self.hour_attendance()[i].working_time());}}
