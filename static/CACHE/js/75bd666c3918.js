...
self.tax_amount=function(){var sum=0;if(self.tax()=='inclusive'){self.particulars.rows().forEach(function(i){if(typeof i.tax_scheme()!='undefined'){var tax_percent=self.tax_scheme_by_id(i.tax_scheme()).percent;var tax_amount=i.amount()*(tax_percent/(100+tax_percent));sum+=tax_amount;}});}else if(self.tax()=='exclusive'){self.particulars.rows().forEach(function(i){if(typeof i.tax_scheme()!='undefined'){var tax_percent=self.tax_scheme_by_id(i.tax_scheme()).percent;var tax_amount=i.amount()*(tax_percent/100);sum+=tax_amount;}});}
return rnum(sum);}
...
self.discount(0);self.amount=ko.computed(function(){var wo_discount=self.quantity()*self.unit_price();var amt=wo_discount-((self.discount()*wo_discount)/100);return amt;});self.validate=function(){if(!self.account()){bs_alert.error('Particular is not allowed to empty');return false;}
