...
function comma_in_thou(x){var neg_num=false;x=x.toString();if(x.indexOf('-')>=0){x=x.replace('-','');neg_num=true;}
res=x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");if(neg_num){output=['-',res].join('');res=output;}
return res;}
function comma_in_indian(x){x=x.toString();var afterPoint='';if(x.indexOf('.')>0)
afterPoint=x.substring(x.indexOf('.'),x.length);x=Math.floor(x);x=x.toString();var lastThree=x.substring(x.length-3);var otherNumbers=x.substring(0,x.length-3);var neg_num=false;if(otherNumbers.indexOf('-')>=0){otherNumbers=otherNumbers.replace('-','');neg_num=true;}
if(otherNumbers!='')
lastThree=','+lastThree;var res=otherNumbers.replace(/\B(?=(\d{2})+(?!\d))/g,",")+lastThree+afterPoint;if(neg_num){output=['-',res].join('');res=output;}
return res;}
...
function bra_to_neg(n){if(n=="-"){n="-";}
else if(n[0]=="-"&&n[1]==","){n=n.slice(2,n.length);n=Math.abs(parseFloat(n));n='('+n+')';n=n.fontcolor("red");console.log(n,typeof n);}}
