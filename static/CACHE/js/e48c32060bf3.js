...
return'0'+s_num;else
return num.toString();}
$(document).ready(function(){$(document).ready(function(){$('.date-picker').datepicker({endDate:'0d'});});vm=new IndividualPayrollVoucherVM(ko_data);ko.applyBindings(vm);$('.change-on-ready').trigger('change');});function IndividualPayrollVoucherVM(data){var self=this;$.ajax({url:'/payroll/employees.json',dataType:'json',async:false,success:function(data){self.employees=data;}});$.ajax({url:'/ledger/accounts.json',dataType:'json',async:false,success:function(data){self.accounts=data;}});self.id=ko.observable();self.message=ko.observable();self.state=ko.observable('standby');self.voucher_no=ko.observable();self.date=ko.observable();self.employee=ko.observable();self.days_worked=ko.observable();self.hours_worked=ko.observable();self.ot_hours_worked=ko.observable();self.day_rate=ko.observable();self.hour_rate=ko.observable();self.ot_hour_rate=ko.observable();self.from_date=ko.observable();self.to_date=ko.observable();self.worked_hours=ko.observable();self.full_attendance_day=ko.observable();self.late_attendance_day=ko.observable();self.half_attendance_day=ko.observable();self.early_leave_day=ko.observable();self.absent_day=ko.observable();self.employee_changed=function(data){var selected_item=$.grep(self.employees,function(i){return i.id==data.employee();})[0];if(!selected_item)return;$.ajax({type:"POST",url:'/payroll/employee-detail/',dataType:'html',data:{'employee':selected_item.id,'end':self.to_date},success:function(data){data=$.parseJSON(data);if(approved){self.worked_hours(data.worked_hours);self.full_attendance_day(data.full_attendance_day);self.late_attendance_day(data.late_attendance_day);self.half_attendance_day(data.half_attendance_day);self.early_leave_day(data.early_leave_day);self.absent_day(data.absent_day);}
else{self.worked_hours(data.unpaid_worked_hours);self.full_attendance_day(data.unpaid_full_attendance_day);self.late_attendance_day(data.unpaid_late_attendance_day);self.half_attendance_day(data.unpaid_half_attendance_day);self.early_leave_day(data.unpaid_early_leave_day);self.absent_day(data.unpaid_absent_day);}
self.from_date(data.start);}});}
self.day_amount=function(){var value=(self.full_attendance_day()+self.half_attendance_day()*0.5+self.early_leave_day()*0.5+self.late_attendance_day()*0.5)*self.day_rate();value=isNaN(value)?0:value;return value;}
self.hour_amount=function(){var value=(self.worked_hours()*self.hour_rate());value=isNaN(value)?0:value;return value;}
...
return filtered_accounts;};for(var k in data){if(data[k])
self[k]=ko.observable(data[k]);}
self.inclusions=new TableViewModel({rows:data.inclusions},IndividualPayrollVoucherRowVM);self.deductions=new TableViewModel({rows:data.deductions},IndividualPayrollVoucherRowVM);self.validate=function(){bs_alert.clear()
...
function IndividualPayrollVoucherRowVM(data){var self=this;self.account=ko.observable();self.amount=ko.observable();for(var k in data)
if(data[k])
self[k]=ko.observable(data[k]);}