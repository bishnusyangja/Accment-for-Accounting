...
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
function InventoryRow(row){var self=this;console.log("thi is");console.log("thisis ",window.vm);console.log("thi is");self.item_id=ko.observable();self.purchase=ko.observable(0);self.sales=ko.observable(0);for(var k in row){if(row[k]!=null)self[k]=ko.observable(row[k]);}
...
else return 0;},self,vm);self.closing=ko.computed(function(){console.log(parseFloat(self.opening()),parseFloat(self.purchase()),parseFloat(self.sales()));return rnum(parseFloat(self.opening())+parseFloat(self.purchase())-parseFloat(self.sales()));},self);self.actual=ko.observable();self.difference=function(){return rnum(self.actual()-self.closing());};}
