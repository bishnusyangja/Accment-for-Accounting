...
window.month=id;var el="#"+id+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});});function attendance_parameter(obj){$.ajax({url:'payroll/attendance-parameter.json',dataType:'json',async:false,success:function(data){console.log(data);obj.parameter=data;}});}
...
return get_hrm(sum);});self.total_day=ko.computed(function(){var sum=0;for(var i=0;i<self.hour_attendance().length;i++){if(self.hour_attendance()[i].working_time()){sum+=get_minutes(self.hour_attendance()[i].working_time());}}
