...
self.discount(0);self.amount=ko.computed(function(){var wo_discount=self.quantity()*self.unit_price();var amt=wo_discount-((self.discount()*wo_discount)/100);return amt;});self.account_changed=function(){var selected=$.grep(vm.accounts,function(i){return i.id==self.account();})[0];if(selected){self.tax_scheme(selected.tax_detail[0].pri_tax_scheme);}}
