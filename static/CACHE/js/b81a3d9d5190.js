...
self.journal_voucher.approve=function(){var ko_data=JSON.parse(ko.toJSON(self));for(var key in ko_data){if(key!='id')
delete ko_data[key];}
var ko_data=ko.toJSON(ko_data);$.ajax({type:"POST",url:'/voucher/journal/approve/',data:ko_data,success:function(msg){if(typeof(msg.error_message)!='undefined'){bs_alert.error(msg.error_message);}
else{bs_alert.success('Approved!');self.status('Approved');self.journal_voucher.state('success');if(msg.redirect_to){window.location=msg.redirect_to;}}}});}
...
self.id(msg.id);}}});}
self.journal_voucher.save=function(item,event){var ko_data=JSON.parse(ko.toJSON(self));delete ko_data.accounts;var ko_data=ko.toJSON(ko_data);self.journal_voucher.state('waiting');var valid=true;var message='';var rows=self.journal_voucher.rows();var selection=$("#journal-voucher > tr");var formdata=new FormData();var file=$('#attachment')[0].files[0];formdata.append('attachment',file);formdata.append('data',ko_data);if(!self.journal_voucher.cr_equals_dr()){message+='Total Dr and Cr amounts don\'t tally!      ';valid=false;}
...
else{bs_alert.success('Saved!');self.deleted_rows=[];self.journal_voucher.state('success');if(msg.id){self.id(msg.id);self.status('Unapproved');}
if(msg.redirect_to){window.location=msg.redirect_to;return;}}},error:function(XMLHttpRequest,textStatus,errorThrown){bs_alert.error('Saving Failed!');self.journal_voucher.state('error');}});}
else
return true;}}
