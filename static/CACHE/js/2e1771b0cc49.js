...
self.table_vm=new TableViewModel(options,GroupPayrollVoucherRowVM);self.check_valid=function(){var valid=false;if(self.date()&&self.voucher_no()&&self.from_date()&&self.to_date()){len=self.table_vm.rows();for(var i=0;i<len.length;i++){if(len[i].employee()&&len[i].rate_day()&&len[i].rate_hour()&&len[i].pay_head()){valid=true;}}}
return valid;}
self.save=function(item,event){if(get_form(event).checkValidity()){if($(get_target(event)).data('continue')){self.continue=true;}
if(!self.check_valid()){bs_alert.error('Fields are not allowed to be empty except payroll tax.')
return false;}
...
self.amount=ko.computed(function(){return(round2z(self.present_days())*round2z(self.rate_day())+round2z(self.present_hours())*round2z(self.rate_hour())).toFixed(2);});self.net=ko.computed(function(){return(self.amount()-round2z(self.payroll_tax())).toFixed(2);});}
