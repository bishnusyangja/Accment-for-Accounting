...
window.month=id;var el="#"+id+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});});function attendance_parameter(obj){$.ajax({url:'/payroll/attendance-parameter.json',dataType:'json',async:false,success:function(data){obj.parameter=data[0];}});}
function employee_ajax(obj){$.ajax({url:'/payroll/employees.json',dataType:'json',async:false,success:function(data){obj.employees=data;}});}
function AttendanceVM(){var self=this;employee_ajax(self);attendance_parameter(self);self.month=ko.observable();self.days=ko.observableArray();self.employee=ko.observable();self.attendance_type=ko.observable();self.hour_attendance=ko.observableArray();self.day_attendance=ko.observableArray();self.total_time=ko.computed(function(){var sum=0;for(var i=0;i<self.hour_attendance().length;i++){if(self.hour_attendance()[i].working_time()){sum+=get_minutes(self.hour_attendance()[i].working_time());}}
return get_hrm(sum);});self.total_day=ko.computed(function(){var sum=0.00;for(var i=0;i<self.day_attendance().length;i++){var att_status=self.day_attendance()[i].attendance_status();if(att_status){if(att_status=='Full Attendance'){sum+=self.parameter.full_att;}else if(att_status=='Late Attendance'){sum+=self.parameter.late_att;}else if(att_status=='Half Attendance'){sum+=self.parameter.half_att;}else if(att_status=='Early Leave'){sum+=self.parameter.early_leave;}else if(att_status=='Absent'){sum+=self.parameter.abs;}}}
sum=(Math.round(sum*100)/100).toString()+' Days';return sum;});self.hour_attendance_status=ko.computed(function(){if(this.attendance_type()=='hour_attendance')return true;},this);self.day_attendance_status=ko.computed(function(){if(this.attendance_type()=='day_attendance')return true;},this);self.if_changed=function(){ajax_request(window.month,self.employee);}
