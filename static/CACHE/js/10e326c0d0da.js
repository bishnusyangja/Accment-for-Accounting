...
var add_file_view=$('.attach_file_field').first();$('.add_file').click(function(){var _parent=$(this).parent('p');var clone=add_file_view.clone();clone.append('<button type="button" class="btn btn-danger pull-right remove-file-attach">X</button>')
clone.find('input').val("");_parent.before(clone);});$(document).on('click','.remove-file-attach',function(){$(this).parent('.attach_file_field').slideUp(400,function(){$(this).remove()});});$('.attachment-form').submit(function(e){e.preventDefault();if(window.FormData){var file_ips=$(this).find('input[type="file"]');var text_ips=$(this).find('.captions');var formdata=new FormData();$.each(text_ips,function(){formdata.append("captions",this.value);});$.each(file_ips,function(){formdata.append("attachments",this.files[0]);});var $this=$(this);var type=$this.data("type");formdata.append("type",type);formdata.append("day",$('#attachment_tabbable').data("journal-day"));$.ajax({url:"/day/save_attachments/",type:"POST",data:formdata,processData:false,contentType:false,success:function(res){var str="";$.each(res,function(){str+='<div class="span3"> <a target="_blank" href="'+this.link+'">'+this.caption+'</a><button class="close delete-attachment" data-type="'+type+'" data-id="'+this.id+'"><span class="icon-trash"></span></button></div>';});$this.find('.row-fluid').append(str);$this.find('.attach_file_field').find("input").val("").end().not(':first').remove();},error:function(){alert("There has been error while processing your request. Please try again!");}});}else{alert("Your browser is too old. Please upgrade to modern browsers like Chrome or Firefox.")}});});function DayJournal(data){var self=this;self.sales_tax=ko.observable();self.state=ko.observable();for(var k in data)
...
self.register_sales_amount=ko.observable(0.00);if(data['register_sales_amount']){self.register_sales_amount(rnum(parseFloat(data['register_sales_amount'])));}
self.register_sales_tax=ko.observable(0.00);if(data['register_sales_tax']){self.register_sales_tax(rnum(parseFloat(data['register_sales_tax'])));}
...
return rnum(parseFloat(empty_to_zero(self.cash_sales.get_total('amount')))+parseFloat(empty_to_zero(self.lotto_sales_dispenser_amount()))+parseFloat(empty_to_zero(total_scratch))+parseFloat(empty_to_zero(self.summary_transfer.total())));}
...
self.save_sales_register=function(){self.day_journal_date=self.date;$.ajax({type:"POST",url:'/day/save_sales_register/',data:ko.toJSON(self),success:function(msg){$('#sales-register-message').html('Saved!');$('#sales-register-message').addClass('success');self.status('Unapproved');},error:function(XMLHttpRequest,textStatus,errorThrown){$('#sales-register-message').html('Saving Failed');$('#sales-register-message').addClass('error');}});}
...
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
function InventoryRow(row){var self=this;self.item_id=ko.observable()
self.opening=function(){if(self.item_id()){var item_temp=vm.inventory_item_by_id(self.item_id()).get_current_stock;return 0;}};self.purchase=ko.observable(0);self.sales=ko.observable(0);self.closing=ko.computed(function(){return rnum(parseFloat(self.opening())+parseFloat(self.purchase())-parseFloat(self.sales()));});self.actual=ko.observable();self.difference=function(){return rnum(self.actual()-self.closing());};for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
...
self[k]=ko.observable(row[k]);}}
function CardSalesRow(row){var self=this;self.amount=ko.observable();self.commission_out=ko.observable();self.net=function(){return rnum(empty_to_zero(self.amount())-empty_to_zero(self.commission_out()));}
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
