...
valid=false;break;}}
var temp_acc=[];for(var i=0;i<self.journal_voucher.rows().length;i++){if(self.journal_voucher.rows()[i].has_particulars()){if(temp_acc.indexOf(self.journal_voucher.rows()[i].account())>=0){console.log(valid);valid=false;console.log(valid);message+='Duplicated Particulars Not Allowed!'
break;}
