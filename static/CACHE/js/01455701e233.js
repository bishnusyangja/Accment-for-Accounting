...
valid=false;break;}}
var temp_acc=[];for(var i=0;i<self.journal_voucher.rows().length;i++){if(self.journal_voucher.rows()[i].has_particulars()){if(self.journal_voucher.rows()[i].account()in temp_acc){console.log(valid);valid=false;console.log(valid);message+='Duplicated Particulars Not Allowed!'
break;}
