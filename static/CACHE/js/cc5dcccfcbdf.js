...
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
function InventoryRow(row){var self=this;console.log("thisis ",vm);self.item_id=ko.observable();self.purchase=ko.observable(0);self.sales=ko.observable(0);for(var k in row){if(row[k]!=null)self[k]=ko.observable(row[k]);}
