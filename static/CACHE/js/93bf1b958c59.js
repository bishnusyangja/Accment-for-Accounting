...
console.log(id);window.month=id;var el="#"+id+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});$("#sele").click(function(){$("#input1").click(function(){$("select.sel").select2();var id=monthNames[objDate.getMonth()]
...
function ajax_request(month,employee){$.ajax({type:"POST",url:'/payroll/attendance-ledger/',dataType:'html',data:{'month':month,'employee':employee},success:function(data){console.log(data);data=$.parseJSON(data);display_day_rows(vm,month,data);},error:function(){console.log('An error occurred!');}});}
