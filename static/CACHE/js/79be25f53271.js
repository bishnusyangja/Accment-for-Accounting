...
len=self.table_vm.rows();for(var i=0;i<len.length;i++){var temp_count=0;for(var j=0;j<len.length;j++){if(len[i].employee()==len[j].employee()){if(temp_count==1){bs_alert.error('Duplicate Employees are not allowed !');return false;}
temp_count+=1;}}
