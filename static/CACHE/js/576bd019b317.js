...
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
function InventoryRow(row){var self=this;self.item_id=ko.observable();self.purchase=ko.observable(0);self.sales=ko.observable(0);self.opening=function(){if(vm.inventory_item_by_id(self.item_id())!=undefined){return vm.inventory_item_by_id(self.item_id()).get_current_stock;}
return 0;};self.closing=ko.computed(function(){console.log(parseFloat(self.opening()),parseFloat(self.purchase()),parseFloat(self.sales()));return rnum(parseFloat(self.opening())+parseFloat(self.purchase())-parseFloat(self.sales()));});self.actual=ko.observable();self.difference=function(){return rnum(self.actual()-self.closing());};for(var k in row){if(row[k]!=null)self[k]=ko.observable(row[k]);}}
