...
return'0'+s_num;else
return num.toString();}
$(document).ready(function(){$('#inv-date').datepicker().data('datepicker');$('#due-date').datepicker({relative_to:'#inv-date'});vm=new PurchaseVoucherViewModel(ko_data);ko.applyBindings(vm);});function TaxOptions(name,id){this.name=name;this.id=id;}
...
else{for(var i=0;i<rows.length;i++){console.log(rows[i].validate())
if(!rows[i].validate()){return false;}}
return true;}}
...
window.location='/voucher/purchase-voucher/'+msg.id;}},error:console.log('error is occured')});}
self.save_continue=function(item,event){var formdata=new FormData();if($('#id_attachment').val())
var file=$('#id_attachment')[0].files[0];formdata.append('data',ko.toJSON(self));formdata.append('attachment',file);$.ajax({type:"POST",url:'/voucher/purchase-voucher/save/',data:formdata,success:function(msg){if(typeof(msg.error_message)!='undefined'){$('#message').html(msg.error_message);}
else{bs_alert.success('Saved!');if(msg.id)
self.id=msg.id;$("#particulars-body > tr").each(function(i){$($("#particulars-body > tr")[i]).addClass('invalid-row');});for(var i in msg.rows){self.particulars.rows()[i].id=msg.rows[i];$($("#particulars-body > tr")[i]).removeClass('invalid-row');}
...
self.tax_amount=function(){var sum=0;if(self.tax()=='inclusive'){self.particulars.rows().forEach(function(i){if(typeof i.tax_scheme()!='undefined'){var tax_percent=self.tax_scheme_by_id(i.tax_scheme()).percent;var tax_amount=i.amount()*(tax_percent/(100+tax_percent));sum+=tax_amount;}});}else if(self.tax()=='exclusive'){self.particulars.rows().forEach(function(i){var tax_percent=self.tax_scheme_by_id(i.tax_scheme()).percent;var tax_amount=i.amount()*(tax_percent/100);sum+=tax_amount;});}
return rnum(sum);}
...
self.particulars.grand_total=ko.computed(function(){if(self.tax()=='exclusive'){return self.sub_total()+self.tax_amount();}
return rnum(self.sub_total());},self);self.approve=function(item,event){if(!self.validate())
