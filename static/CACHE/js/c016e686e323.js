...
$(save_button).attr('disabled',true);if(window.FormData){var file_ips=$(this).find('input[type="file"]');var text_ips=$(this).find('.captions');var formdata=new FormData();$.each(text_ips,function(){formdata.append("captions",this.value);});$.each(file_ips,function(){formdata.append("attachments",this.files[0]);});var $this=$(this);var type=$this.data("type");formdata.append("type",type);formdata.append("day",$('#attachment_tabbable').data("journal-day"));$.ajax({url:"/day/save_attachments/",type:"POST",data:formdata,processData:false,contentType:false,async:false,success:function(res){var str="";$.each(res,function(){str+='<div class="span3"> <a target="_blank" href="'+this.link+'">'+this.caption+'</a><button class="close delete-attachment" data-type="'+type+'" data-id="'+this.id+'"><span class="icon-trash"></span></button></div>';});$this.find('.row-fluid').append(str);$this.find('.attach_file_field').find("input").val("").end().not(':first').remove();},error:function(){alert("There has been error while processing your request. Please try again!");}});$(save_button).attr('disabled',false);}else{alert("Your browser is too old. Please upgrade to modern browsers like Chrome or Firefox.")}});});function clearFileInput(id){var oldInput=document.getElementById(id);var newInput=document.createElement("input");newInput.type="file";newInput.id=oldInput.id;newInput.name=oldInput.name;newInput.className=oldInput.className;newInput.style.cssText=oldInput.style.cssText;oldInput.parentNode.replaceChild(newInput,oldInput);}
...
self.actual_sales_amount=function(){var total_scratch=self.scratch_off_total();var result=rnum(parseFloat(empty_to_zero(self.cash_sales.get_total('amount')))+parseFloat(empty_to_zero(self.lotto_sales_dispenser_amount()))+parseFloat(empty_to_zero(total_scratch))+parseFloat(empty_to_zero(self.summary_transfer.total())));return result;}
...
return filtered_accounts.sort(function(a,b){return compareStrings(a.name,b.name);});};self.accounts_by_category_list=function(categories,is_or){var filtered_accounts=[];for(var i=0;i<categories.length;i++){var acc=self.accounts_by_category(categories[i],is_or);for(var j=0;j<acc.length;j++){filtered_accounts.push(acc[j]);}}
return filtered_accounts.sort(function(a,b){return compareStrings(a.name,b.name);});};function remove_item(list,item){while(list.indexOf(item)>=0){list.splice(list.indexOf(item),1);}
return list;}
...
return temp_accounts.sort(function(a,b){return compareStrings(a.name,b.name);});};self.accounts_except_category=function(categories,is_or){var filtered_accounts=[];for(var i in self.accounts){var account_categories=self.accounts[i].categories
if(typeof categories==='string'){if($.inArray(categories,account_categories)==-1){filtered_accounts.push(self.accounts[i]);}}else if(typeof is_or!='undefined'){if(!intersection(categories,account_categories).length){filtered_accounts.push(self.accounts[i]);}}else{if(!compare_arrays(categories,account_categories)){filtered_accounts.push(self.accounts[i]);}}}
return filtered_accounts.sort(function(a,b){return compareStrings(a.name,b.name);});};self.sales_sans_lotto=function(){var sales_accounts=self.accounts_by_category('Sales');var accounts=[];for(var i in sales_accounts){if(sales_accounts[i].name!='Lotto Sales'&&sales_accounts[i].name!='Scratch Off Sales')
...
for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
function InventoryRow(row){var self=this;self.item_id=ko.observable();self.purchase=ko.observable(0);self.sales=ko.observable(0);self.opening=function(){if(self.item_id()){var item_temp=vm.inventory_item_by_id(self.item_id()).opening;return item_temp;}};self.closing=function(){return rnum(parseFloat(self.opening())+parseFloat(self.purchase())-parseFloat(self.sales()));};self.actual=ko.observable();self.difference=function(){return rnum(self.actual()-self.closing());};for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
...
return element;})[0];return rnum(cash_account.opening);};self.inward=function(root){var total=0;total+=parseFloat(root.actual_sales_amount());total+=parseFloat(root.register_sales_tax());if(root.card_sales.rows()[0].amount()){total-=parseFloat(root.card_sales.rows()[0].amount());}
...
return out;}
return obj;}
function compareStrings(a,b){if(typeof a=='undefined')a='';if(typeof b=='undefined')b='';a=a.toLowerCase();b=b.toLowerCase();return(a<b)?-1:(a>b)?1:0;}