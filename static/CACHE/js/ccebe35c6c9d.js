...
valid=false;break;}}
var temp_acc=[];for(var i=0;i<self.journal_voucher.rows().length;i++){if(self.journal_voucher.rows()[i].has_particulars()){console.log(self.journal_voucher.rows()[i].account());if(self.journal_voucher.rows()[i].account()in temp_acc){valid=false;message+='Duplicated Particulars Not Allowed!'
break;}
