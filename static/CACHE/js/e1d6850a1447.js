...
self.party=ko.observable();self.description=ko.observable();for(var k in data)
...
$.ajax({type:"POST",url:'/voucher/invoice/save/',data:ko.toJSON(self),success:function(msg){if(typeof(msg.error_message)!='undefined'){bs_alert.error(msg.error_message);}
...
return rnum(sum);}
self.total_amount=0;self.grand_total=function(){if(self.tax()=='exclusive'){self.total_amount=self.sub_total()+self.tax_amount();}
...
self.customer_changed=function(vm){var selected_obj=$.grep(self.customers,function(i){return i.id==vm.party;})[0];self.party_address(selected_obj.address);}}
...
self.discount(0);self.amount=ko.computed(function(){var wo_discount=self.quantity()*self.unit_price();var amt=wo_discount-((self.discount()*wo_discount)/100);return amt;});}