...
window.location='/voucher/fixed-asset/'+self.id();$("#rows-body > tr").each(function(i){$($("#rows-body > tr")[i]).addClass('invalid-row');});for(var i in msg.rows1){self.table_vm.rows()[i].id=msg.rows1[i];$($("#rows-body > tr")[i]).removeClass('invalid-row');}
...
self.id(msg.id);window.location='/voucher/fixed-asset/'+self.id();}}});}
else
return true;}
