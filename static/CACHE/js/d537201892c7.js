$(document).ready(function(){$("#selectChoice").select2();hide_sub_form();var category=get_category();request_ajax(category);$("form select[name='category']").on("click",function(){console.log('On click categoruy');var category=get_category();request_ajax(category);});show_for_update(scenario);});function category_form(check){if(check!==''){var cat_form="#"+check;console.log(cat_form);hide_sub_form();$(cat_form).show();}
...
function request_ajax(category){if(category!=''){$.ajax({url:'/ledger/detect_category/',type:'POST',data:{'category':category},dataType:'html',success:function(result){if(result!==0){console.log('ajad successful')
category_form(result);}},error:function(){console.log("An error occurred !");}});}}
