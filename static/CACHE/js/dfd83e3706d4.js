...
window.month=id;var el="#"+mnth+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});$("#sele").click(function(){$("#input1").click(function(){$("select.sel").select2();var id=monthNames[objDate.getMonth()]
window.month=id;var el="#"+mnth+"-tab";$(el).parent().attr('class','active');ajax_request(id,vm.employee);});});});function employee_ajax(obj){$.ajax({url:'/payroll/employees.json',dataType:'json',async:false,success:function(data){obj.employees=data;}});}
