...
else return 0;};self.purchase=ko.observable();self.sales=ko.observable();self.closing=ko.computed(function(){console.log(parseFloat(self.opening()),parseFloat(self.purchase()),parseFloat(self.sales()));return rnum(parseFloat(self.opening())+parseFloat(self.purchase())-parseFloat(self.sales()));});self.actual=ko.observable();self.difference=function(){return rnum(self.actual()-self.closing());};for(var k in row){if(row[k]!=null)
self[k]=ko.observable(row[k]);}}
