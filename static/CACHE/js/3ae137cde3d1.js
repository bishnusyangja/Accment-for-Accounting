...
var account_list=[];for(each in self.inclusions.rows()){if(account_list.indexOf(self.inclusions.rows()[Number(each)].account())==-1)
account_list.push(self.inclusions.rows()[Number(each)].account());else{bs_alert.error('There same ledgers in multiple rows of either deductions or inclusions. Please make sure any ledger is not repeated.');return false;}}
account_list=[];for(each in self.deductions.rows()){if(account_list.indexOf(self.deductions.rows()[Number(each)].account())==-1)
account_list.push(self.deductions.rows()[Number(each)].account());else{bs_alert.error('There same ledgers in multiple rows of either deductions or inclusions. Please make sure any ledger is not repeated.');return false;}}
return true;}
...
return false;if(!self.check_valid()){bs_alert.error("Fields are not allowed to be empty.");return false;}
