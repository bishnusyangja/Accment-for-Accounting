...
valid=false;break;}}
var temp_acc=[];for(var i=0;i<self.journal_voucher.rows().length;i++){if(self.journal_voucher.rows()[i].has_particulars()){if(self.journal_voucher.rows()[i].account()in temp_acc){console.log(i);valid=false;message+='Duplicated Particulars Not Allowed!'
break;}
else{temp_acc.push(self.journal_voucher.rows()[i].account());}}}
console.log(valid,temp_acc);if(!valid){self.journal_voucher.state('error');bs_alert.error(message);return false;}
