function init_select2(element, callback) {
    if ($(element).data('url')) {
        if ($(element).data('name')) {
            ...
            //console.log($(element).data('url'), $('#appended-link' + $(element).data('counter')).length);
         ...
         var temp = "a[id='appended-link" + $(element).data('counter') + "']";
         $(temp).not(":last").remove();
       }
...
function rnum(n) {
    if (!isAN(n))
        return '';
//     //round-off
    var b = Math.round(n * Math.pow(10, global_settings.decimal_places)) / Math.pow(10, global_settings.decimal_places);
    var a = b.toString();
    var reg = "/^(?:\d*\.\d{1,"+global_settings.decimal_places.toString()+"}|\d+)$/";
    a.match(reg);
    return parseFloat(a);
}
...
function znum(n){
    ...
    else
    {
        var s = n
        if (s.indexOf('.') == -1) s += '.';
        while (s.length < s.indexOf('.') + global_settings.decimal_places+1) s += '0';
        return s;
    }

}
function znum_for_fuels(n){
    if(typeof n == 'undefined' || n == null)
        return '';
    else if(n == ''){
        return ''
...
function zero_to_dash(n){
    if (n == '' || n == '0' || typeof n == 'undefined' || n == '0.00' || n == '0.000' || n == '-')
        return '-';
    else return n;
}

/*function bra_to_neg(n){
    if(n=="-"){
    n = "-";
    }
    else if(n[0]=="-" && n[1]==","){
...
function TableViewModel(options, row_model, id) {
    ...
    if (typeof(options.save_to_url) != 'undefined') {
        self.save = function (model, e) {
            self.state('waiting');
            var el = get_target(e);
            if(id!='undefined') $(id).attr('disabled',true);
            $.ajax({
                type: "POST",
                ...
                success: function (msg) {
                    ...
                    self.deleted_rows = [];
                    if(id!='undefined') $(id).attr('disabled',false);
                },
